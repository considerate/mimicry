...
function newcarry(rng, sizes, batchsize=1) :: LSTMState
    ...
    return (
            ...
            (Lux.zeros32(rng, b, batchsize), Lux.zeros32(rng, b, batchsize)) .|> Lux.gpu,
            (Lux.zeros32(rng, c, batchsize), Lux.zeros32(rng, c, batchsize)) .|> Lux.gpu,
           )
end
...
function logloss(activations :: CuMatrix32, sampled::Int) :: Float32
    return -sum(activations[sampled, :])
end
...
function cars()
    ...
    loop() do frame, csv
        ...
        after_update = time_ns()
        update_duration = (after_update - before_update) / 1e9
        # println(string(update_duration, "s"))
